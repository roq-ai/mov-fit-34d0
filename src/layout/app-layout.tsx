...
import { getWorkoutErs } from 'apiSdk/workout-ers';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['overlord', 'user'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<WorkoutErInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/workout-ers` : null),
    () => getWorkoutErs({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Workout Er',
      path: isTenantUser ? `/workout-ers/view/${data?.[0]?.id}` : '/workout-ers',
      entity: 'workout_er',
      service: AccessServiceEnum.PROJECT,
      icon: FiActivity,
    },
    { name: 'Exercise', path: '/exercises', entity: 'exercise', service: AccessServiceEnum.PROJECT, icon: FiClock },
    { name: 'Media', path: '/media', entity: 'media', service: AccessServiceEnum.PROJECT, icon: FiFilm },
    { name: 'Workout', path: '/workouts', entity: 'workout', service: AccessServiceEnum.PROJECT, icon: FiHeart },
